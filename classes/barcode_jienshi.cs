   ...
   public  class barcode_jienshi
    {
        ...
        public barcode_jienshi(string mip, int mport, string mLip, int mLport, string mdes,string loggs)
        {
            MIP = mip;
            Mport = mport;
            localIP = mLip;
        ...
        public EmRes connect()
        {
            ...
            IP = System.Net.IPAddress.Parse(MIP);
            try
            {
        ...
        {          
            if (IsConnected)
            {
                ...
                {
                    Logger.Error($"{mdescrible}发送信息{strs}失败:{ex.Message}");
                    IsConnected = false;
                    return EmRes.Error;
                }
        ...
        /// 第一次扫码
        /// </summary>
        /// <param name="cos">返回的二维码</param>
        /// <param name="cordcs">二维码个数</param>
        /// <param name="ts">超时</param>
        /// <returns>true:一次扫码结果OK,false:一次扫码扫描到东西了，结果失败</returns>
        public bool firstScan(ref string cos, int cordcs, int ts = 50)
        {
            cos = "";
            int a = 0;
            Sendb("LON\u000d");
            ...
            while (ReceiveStringData.Length < 2)
            {
                ...
                if (a > ts)
                {
                    Logger.Error($"{mdescrible}一次扫码接收数据超时.一次扫码OK");
                    return true;
                }
            ...
            if (cordcs == 1)
            {
                if (ReceiveStringData.Contains("ERROR"))
                {
                    Logger.Info($"{mdescrible}一次扫码接收到数据:{ReceiveStringData}。一次扫码OK");
                    cos = ReceiveStringData;
                    return true;
                }
                else
                {
                    Logger.Error($"{mdescrible}一次扫码接收到数据{ReceiveStringData}。一次扫码NG");
                    cos = ReceiveStringData;
                    return false;
                }
            }
            ...
            {
                if (ReceiveStringData.Contains("ERROR,ERROR"))
                {
                    Logger.Info($"{mdescrible}一次扫码接收到数据:{ReceiveStringData}。一次扫码OK");
                    cos = ReceiveStringData;
                    return true;
            ...
            {
                if (ReceiveStringData.Contains("ERROR,ERROR,ERROR"))
                {
                    Logger.Info($"{mdescrible}一次扫码接收到数据:{ReceiveStringData}。一次扫码OK");
                    cos = ReceiveStringData;
                    return true;
            ...
            {
                if (ReceiveStringData.Contains("ERROR,ERROR,ERROR,ERROR"))
                {
                    Logger.Info($"{mdescrible}一次扫码接收到数据:{ReceiveStringData}。一次扫码OK");
                    cos = ReceiveStringData;
                    return true;
        ...
        /// 二次扫码
        /// </summary>
        /// <param name="CORD">返回的</param>
        /// <param name="cordcs">二维码的个数</param>
        /// <param name="ts">超时时间</param>
        /// <returns>true:扫码OK，false：扫码NG</returns>
        public bool SeconScan(ref string CORD, int cordcs, int ts = 60)
        {
            int a = 0;
            if(Sendb("LON\u000d")!=EmRes.Succeed)
            {
            ...
            while (ReceiveStringData.Length < 2)
            {
                ...
                if (a > ts)
                {
                    Logger.Error($"{mdescrible}二次扫码接收数据超时");
                    CORD = "";
                    return false;
                }
                if (ReceiveStringData.Length > 1)
                {
                    Thread.Sleep(20);
                    break;
                }
            }
            string[] bb = ReceiveStringData.Split(',');
            if (bb.Length == cordcs)
            {
                if (!ReceiveStringData.Contains("ERROR"))
                {
                    Logger.Info($"{mdescrible}二次扫码接收数据完成{ReceiveStringData}");
                    CORD = ReceiveStringData;
                    return true;
                }
                else
                {
                    Logger.Info($"{mdescrible}二次扫码失败，接收数据：{ReceiveStringData}");
                    CORD = ReceiveStringData;
                    return false;
                }
            }

            Logger.Error($"{mdescrible}二次扫码接收数据NG:{ReceiveStringData}");
            CORD = ReceiveStringData;
            return false;
        }
